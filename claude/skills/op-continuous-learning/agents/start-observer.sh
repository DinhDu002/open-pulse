...
PYTHON_CMD="${OP_CL_PYTHON_CMD:-}"

# ─────────────────────────────────────────────
# Configuration
# ─────────────────────────────────────────────

CONFIG_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)/cl"
if [ -n "${OP_CL_CONFIG:-}" ]; then
  CONFIG_FILE="$OP_CL_CONFIG"
else
...
SENTINEL_FILE="${OP_CL_OBSERVER_SENTINEL_FILE:-${PROJECT_ROOT:-$PROJECT_DIR}/.observer.lock}"
...
if [ -f "$CONFIG_FILE" ]; then
  if [ -z "$PYTHON_CMD" ]; then
    ...
    _config=$(OP_CL_CONFIG="$CONFIG_FILE" "$PYTHON_CMD" -c "
import json, os
with open(os.environ['OP_CL_CONFIG']) as f:
    cfg = json.load(f)
...
case "$ACTION" in
  ...
  start)
    ...
    nohup env \
      ...
      OP_CL_IS_WINDOWS="$IS_WINDOWS" \
      OP_CL_OBSERVER_PROMPT_PATTERN="$OP_CL_OBSERVER_PROMPT_PATTERN" \
    ...
    if tail -n +"$((start_line + 1))" "$LOG_FILE" 2>/dev/null | grep -E -i -q "$OP_CL_OBSERVER_PROMPT_PATTERN"; then
